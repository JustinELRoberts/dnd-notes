---
publish: true
created: 2026-03-27T02:48:22.058Z
modified: 2026-05-25T14:05:54.627Z
published: 2026-05-25T14:05:54.627Z
LocationType: Building
aliases:
tags:
---

## Map

```zoommap
imageBases:
  - path: Campaigns/Under The Ash/Locations/_img/Arena of Vel'Therin Expanse Map.png
    name: Arena of Vel'Therin Expanse
markers: Campaigns/Under The Ash/Locations/_markers/Arena of Vel'Therin Expanse.markers.json
markerLayers:
  - Default
minZoom: 0.26
maxZoom: 1
width: 540px
height: 532px
resizable: false
render: canvas
id: Arena of Vel'Therin Expanse-map
```

## Description

A fighting arena in [[Campaigns/Under The Ash/Locations/Evermore/The Vel’Therin Expanse/Vel Rau'dan/index|Vel Rau'dan]]. We killed (an old, weakened) [[behir|Behir]] here. On the outside, there are three entrances, each for different purposes:

1. To participate in fighting
2. To observe the fighting
3. The prison where fighters are held

There are three tiers of monsters that one can fight here, each tier providing a range of betting odds:

- A: 50-100:1
- B: 10-45:1
- C: 5-10:1
- D: 0.25-5:1
