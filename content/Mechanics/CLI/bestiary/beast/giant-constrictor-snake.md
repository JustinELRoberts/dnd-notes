---
publish: true
aliases:
  - Giant Constrictor Snake
created: 2026-03-29T21:53:55.613-04:00
modified: 2026-03-29T21:53:55.613-04:00
published: 2026-03-29T21:53:55.613-04:00
tags:
  - ttrpg-cli/compendium/src/5e/mm
  - ttrpg-cli/monster/cr/2
  - ttrpg-cli/monster/environment/desert
  - ttrpg-cli/monster/environment/forest
  - ttrpg-cli/monster/environment/swamp
  - ttrpg-cli/monster/environment/underdark
  - ttrpg-cli/monster/environment/underwater
  - ttrpg-cli/monster/size/huge
  - ttrpg-cli/monster/type/beast
cssclasses:
  - json5e-monster
obsidianUIMode: preview
statblock: true
statblock-link: "#^statblock"
name: Giant Constrictor Snake
ac: 12
hp: 60
modifier: 2
hit_dice: 8d12 + 8
cr: "2"
stats:
  - 19
  - 14
  - 12
  - 1
  - 10
  - 3
source:
  - MM
---

# Giant Constrictor Snake

```ad-statblock
title: Giant Constrictor Snake
![](/Mechanics/CLI/bestiary/beast/token/giant-constrictor-snake.webp#token)
*Huge beast, Unaligned*

- **Armor Class** 12 
- **Hit Points** 60 (8d12 + 8) 
- **Speed** 30 ft., swim 30 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|19 (+4)|14 (+2)|12 (+1)| 1 (-5)|10 (+0)| 3 (-4)|

- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** [Perception](/Mechanics/CLI/skills.md#Perception) +2
- **Senses** [blindsight](/Mechanics/CLI/senses.md#Blindsight) 10 ft., passive Perception 12
- **Languages** —
- **Challenge** 2

## Actions

***Bite.*** *Melee Weapon Attack:* `+6` to hit, reach 10 ft., one creature. *Hit:* 11 (`2d6 + 4`) piercing damage.

***Constrict.*** *Melee Weapon Attack:* `+6` to hit, reach 5 ft., one creature. *Hit:* 13 (`2d8 + 4`) bludgeoning damage, and the target is [grappled](/Mechanics/CLI/conditions.md#Grappled) (escape DC 16). Until this grapple ends, the creature is [restrained](/Mechanics/CLI/conditions.md#Restrained), and the snake can't constrict another target.
```

^statblock

## Environment

underwater, underdark, forest, swamp, desert

## Sources

_Monster Manual p. 324. Available in the <span title='Systems Reference Document (5.1)'>SRD</span> and the Basic Rules (2014)_
